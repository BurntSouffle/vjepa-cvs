"""
LoRA Fine-Tuning Training Script (Exp10)
========================================
Low-Rank Adaptation of V-JEPA for CVS classification.

Key features:
  - Freeze ALL original V-JEPA weights
  - Add trainable LoRA adapters to attention layers
  - Train only LoRA adapters + task heads
  - Much more parameter efficient (~1-2M vs 25M trainable)
"""
...
def main(config_path: str = "configs/exp10_lora.yaml"):
    ...
    warmup_ratio = training_cfg.get("warmup_epochs", 2) / training_cfg["epochs"]
    ...
    logger.info(f"Scheduler: cosine with {warmup_ratio*100:.0f}% warmup")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LoRA fine-tuning for V-JEPA CVS model")
    parser.add_argument("--config", type=str, default="configs/exp10_lora.yaml")
